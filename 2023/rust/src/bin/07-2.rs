...
fn get_hand_score(hand: &str) -> usize {
    ...
    let num_jacks = counts.remove(&'J').unwrap_or(0);
    let mut strengths: Vec<usize> = counts.into_values().collect();
    strengths.sort_by_key(|k| Reverse(*k));
    let mut score = 1;
    if num_jacks == 5 {
        score = 7;
    } else if strengths[0] + num_jacks == 5 {
        score = 7;
    } else if strengths[0] + num_jacks == 4 {
        score = 6;
    } else if strengths[0] + num_jacks == 3 && strengths[1] == 2 {
        score = 5;
    } else if strengths[0] + num_jacks == 3 && strengths[1] == 1 {
        score = 4;
    } else if strengths[0] + num_jacks == 2 && strengths[1] == 2 {
        score = 3;
    } else if strengths[0] + num_jacks == 2 && strengths[1] == 1 {
        score = 2;
    }
    score
}
